...
func (a *Api) addAvailability(w http.ResponseWriter, r *http.Request) {
	...
	if !a.Database.HasValidLicense(refereeId) {
...
func (a *Api) requestNewPhone(w http.ResponseWriter, r *http.Request) {
	...
	tokenHex, err := a.Database.CreatePhoneChangeToken(userID, *payload.Phone)
	if err != nil {
		fail(w, err)
		return
	}
...
func (a *Api) assignReferee(w http.ResponseWriter, r *http.Request) {
	...
	if !a.Database.HasValidLicense(*payload.RefereeID) {
		fail(w, types.ErrForbidden)
		return
	}
...
func (a *Api) updateUserProfile(w http.ResponseWriter, r *http.Request) {
	...
	ok(w, http.StatusOK, "user profile updated", nil)
}

// applyReferee allows a user with complete profile data to become a referee (pending license)
func (a *Api) applyReferee(w http.ResponseWriter, r *http.Request) {
	userID := r.Context().Value(UserIdKey).(int)

	if err := a.Database.ApplyReferee(userID); err != nil {
		fail(w, err)
		return
	}

	ok(w, http.StatusOK, "applied for referee", nil)
}
